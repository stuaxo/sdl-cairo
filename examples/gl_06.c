...
int main ()
{
   ...
   while( ! quit )
   {
      ...
      SDL_GetRendererOutputSize( renderer, &renderer_xc, &renderer_yc );
      //---
      cairo_set_source_rgb( cr, 0.690196078, 0.274509804, 0.760784314 );
      cairo_arc( cr, renderer_xc / 2, renderer_yc / 2, 256.0, 0, 2 * M_PI );
      cairo_fill( cr );
      ...
      cairo_clip( cr );
      cairo_set_source_rgb( cr, 1.0, 1.0, 1.0 );
      cairo_rectangle( cr, 0, ( renderer_yc - 220 ) / 2, renderer_xc, 180 );
      cairo_fill( cr );

      cairo_set_source_rgb( cr, 0.0, 0.0, 0.0 );
      cairo_rectangle( cr, 0, ( renderer_yc - 280 ) / 2, renderer_xc, 30 );
      cairo_rectangle( cr, 0, ( renderer_yc + 140 ) / 2, renderer_xc, 30 );
      cairo_fill( cr );
      ...
      x = ( renderer_xc  - te.width ) / 2;
      y = ( renderer_yc + te.height / 2 ) / 2;
